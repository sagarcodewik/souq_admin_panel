...
import { fetchChatUsersByUserId, fetchMessagesByChatId } from '../../../redux/slice/chats'
...
const Chats = () => {
  const { t } = useTranslation('driverChats')
  const dispatch = useDispatch()
  const [userId, setUserId] = useState(null)
  const [selectedUser, setSelectedUser] = useState(null)
  const [loading, setLoading] = useState(true)

  const chatUsers = useSelector((state) => state.chats.chatUsers || [])
  const { fetchStatus, error } = useSelector((state) => state.chats)
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        setUserId(uid)
        dispatch(fetchChatUsersByUserId({ userId: uid, role: 3 })).finally(() => setLoading(false))
      } catch (err) {
